COMPONENT('grid', 'filter:true;external:false;fillcount:50;filterlabel:Filtering values ...;boolean:true|on|yes;pluralizepages:# pages,# page,# pages,# pages;pluralizeitems:# items,# item,# items,# items;pagination:false;rowheight:30', function(self, config) {
	...
	self.make = function() {
		...
		self.html('<div class="ui-grid-loading hidden"><div></div></div><div class="ui-grid"><table class="ui-grid-header"><thead></thead></table><div class="ui-grid-scroller"><table class="ui-grid-data"><thead></thead><tbody></tbody></table></div></div>' + (config.pagination ? '<div class="ui-grid-footer hidden"><div class="ui-grid-meta"></div><div class="ui-grid-pagination"><button class="ui-grid-button" name="first"><i class="fa fa-angle-double-left"></i></button><button class="ui-grid-button" name="prev"><i class="fa fa-angle-left"></i></button><div class="page"><input type="text" maxlength="5" class="ui-grid-input" /></div><button class="ui-grid-button" name="next"><i class="fa fa-angle-right"></i></button><button class="ui-grid-button" name="last"><i class="fa fa-angle-double-right"></i></button></div><div class="ui-grid-pages"></div></div></div>' : ''));
...
COMPONENT('actionbutton', 'icons:floppy-o|refresh|check|times;timeout1:10000;timeout2:3000;colors:#20c2f7|green|red;fullwidth:false', function(self, config){
	var icons;
	var colors;
	var defaulticon = '';
	var defaultcolor;
	...
	self.make = function() {
		var text = self.html();
		self.empty();
		self.aclass('ui-actionbutton');
		icons = config.icons.split('|');
		colors = config.colors.split('|');
		defaultcolor = self.css('background-color');
		config.fullwidth && self.css('width', '100%');
		self.append('<i class="fa{0}"></i>{1}'.format(icons[0] ? ' fa-' + icons[0] : '', text ? '<span>{0}</span>'.format(text) : ''));
		$icon = self.find('i');
		$span = self.find('span');
		self.buttondefault();
    ...
    self.buttondefault = function(type) {
    	self.css('background-color', defaultcolor);
    	self.attr('disabled', false);
		$icon.rclass2('fa-');
		$span.tclass('hidden', false);
    	icons[0] && $icon.aclass('fa-' + icons[0]);
    };

    self.buttonloading = function(type) {
    	self.element.width(self.element.width());
    	self.css('background-color', colors[0] || defaultcolor);
		$icon.rclass2('fa-');
		$icon.aclass('fa-spin fa-' + icons[1]);
		$span.tclass('hidden', true);
    ...
    self.buttonstatus = function(success) {
    	self.css('background-color', success ? (colors[1] || defaultcolor) : (colors[2] || defaultcolor));
    	clearTimeout(timeout);
		$icon.rclass2('fa-');
		$icon.aclass('fa-' + (success ? icons[2] : icons[3]));
		setTimeout(self.buttondefault, config.timeout2);
    };

	self.setter = function(value, path, type) {
		type === 1 && setTimeout(function(){ self.buttonstatus(value); }, 500);
	};
});
